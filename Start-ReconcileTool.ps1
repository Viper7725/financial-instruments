...
$outputRow = Add-FieldRow -LabelText (U '\u5BF9\u8D26\u7ED3\u679C\u4FDD\u5B58\u4F4D\u7F6E') -Top 146 -ButtonText (U '\u53E6\u5B58\u4E3A')
$shareStatementRow = Add-FieldRow -LabelText (U '\u8D22\u52A1\u5206\u6210\u660E\u7EC6\u8868') -Top 192 -ButtonText (U '\u9009\u62E9')
...
$statusLabel.Location = New-Object System.Drawing.Point(160, 337)
...
function Get-DefaultOutputPath {
    ...
    $periodInfo = Get-OutputPeriodInfo -Path $InputPath
    ...
    $fileName = if (-not [string]::IsNullOrWhiteSpace($periodInfo.DisplayLabel)) {
        ((U '\u5BF9\u8D26\u7ED3\u679C') + "-" + $periodInfo.DisplayLabel + ".xlsx")
    } else {
        ((U '\u5BF9\u8D26\u7ED3\u679C') + ".xlsx")
    }
...
$saveDialog.FileName = ((U '\u5BF9\u8D26\u7ED3\u679C') + ".xlsx")
...
$inputRow.Button.Add_Click({
    if ($inputDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ([string]::IsNullOrWhiteSpace($outputRow.TextBox.Text)) {
            $outputRow.TextBox.Text = Get-DefaultOutputPath -InputPath $inputDialog.FileName
        }
    }
})
...
$form.Add_Shown({
    if ([string]::IsNullOrWhiteSpace($outputRow.TextBox.Text)) {
        $outputRow.TextBox.Text = Get-DefaultOutputPath -InputPath $null
    }
    $form.Activate()
})
